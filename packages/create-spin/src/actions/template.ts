...
export async function template(ctx: Pick<Context, 'template' | 'prompt' | 'dryRun' | 'exit'>) {
  if (!ctx.template) {
    const { template: template } = await ctx.prompt({
      ...
      initial: 'standalone',
      choices: [
        { value: 'standalone', label: 'Standalone webapp, powered by Astro', hint: '(recommended)' },
        { value: 'csr-astro-react', label: 'Astro Frontend app (React)' },
        { value: 'csr-micro-frontend-react', label: 'Micro-frontend app (React)' },
        { value: 'micro-frontend-es', label: 'Micro-frontend app (VanillaJS)' },
        { value: 'micro-frontend-shell', label: 'Micro-frontend app (Shell)' },
        ...
        { value: 'ssr-astro-react', label: 'Astro Frontend app (React SSR)' },
